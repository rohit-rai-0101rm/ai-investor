...
# ===== FREE ALTERNATIVE (Groq, OpenAI-compatible API) =====
...
def get_structured_completion(
    ...
    # ===== FREE ALTERNATIVE (Groq, JSON mode instead of structured outputs) =====
    model = model or os.getenv("GROQ_CHAT_MODEL", "llama-3.1-8b-instant")
    ...
    messages = [
        {
            "role": "system",
            "content": "You are an expert financial analyst. Always respond with valid JSON only, no extra text."
        },
        {
            "role": "user",
            "content": prompt
